...
@cuda.jit
def initPop_UBQP( rng_states, D, tColor):
    ...
    if (d < D):
        ...
        for i in range(size):
            tColor[d,i] = int(2 * xoroshiro128p_uniform_float32(rng_states, d))
...
def preprocess_PLSE_instance(filepath, nameGraph, maxCol, kernalization):
    ...
    print("nbNodeswithOneColor")
    print(nbNodeswithOneColor)
    
    print("dico_D")
    print(dico_D)

    print("len(list_nodes_one_color)")
    print(len(list_nodes_one_color))
    ...
    if(kernalization):
        ...
        for node in list_nodes_one_color:
            ...
            if(len(D) == 1):
                ...
                list_removed_node.append(node)

                for j in range(n):

                    if(A[node,j] == 1 and (j not in list_nodes_one_color) ):
...
@cuda.jit
def computeFitness_UBQP( D, A, tColor, score):
